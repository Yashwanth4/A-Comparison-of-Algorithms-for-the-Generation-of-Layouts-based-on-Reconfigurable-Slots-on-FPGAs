...
# this function reads the file and gets block_sizes
...
def read_device_file(file_path):
    ...
    fpga_width = int(contents.split('fpga_width = ')[1].split('\n')[0])
    fpga_height = int(contents.split('fpga_height = ')[1].split('\n')[0])
    preplaced_blocks = json.loads(contents.split('preplaced_blocks = ')[1].split('\n')[0].rstrip(',\n'))

    return fpga_width, fpga_height, preplaced_blocks
...
block_sizes = read_blocks(filename)
if device == "Zynq7020":
    fpga_width,fpga_height, preplaced_blocks = read_device_file("Zynq7020.txt")
...
if device == "Zynq7030":
    fpga_width, fpga_height, preplaced_blocks = read_device_file("Zynq7030.txt")
    final_layout = simulated_annealing(fpga_width, fpga_height, block_sizes, preplaced_blocks, T, T_min, alpha_norm,
                                       alpha_min, alpha_max, alpha_factor, inner_loop, target_acceptance,
                                       acceptance_window)
    final_acceptance = final_accept(final_layout)
